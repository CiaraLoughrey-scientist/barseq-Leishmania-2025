...
#Hullahalli dataset used for this re-analysis#
...
#new load of dataframe#
barcode_counts5 <- read.csv("1_to_54_OrderedFrequencies.csv")
...
INPUTS_VEC <- c("in.0bp5", "in.0ap4", "in.0a", "in.0b", "in.0c")

INPUTS_VEC2 <- c("INPUT_in.0bp5", "INPUT_in.0ap4", "INPUT_in.0a", "INPUT_in.0b", "INPUT_in.0c")

OUTPUTS_VEC <- c("OUTPUT_in.0bp5", "OUTPUT_in.0ap4", "OUTPUT_in.0a", "OUTPUT_in.0b", "OUTPUT_in.0c")
...
ggsave("FFP_HISTOGRAM_HULLAHALLI_facet.png", width  = 6, height = 10)
ggsave("FFP_HISTOGRAM_HULLAHALLI_facet.pdf", width  = 6, height = 10)
...
ggplot(subset(df_long2, direction != "NA"), aes(x = log(new_FP))) + 
  ...
  theme_bw() + theme(legend.title = element_text(size = 20),
                     ...
                     strip.text = element_text(size = 20)) +
  facet_wrap(~direction, nrow = 2, , labeller = as_labeller(c("1TRUE" = "True: A to B", "2BACK" = "Back: B to A")))
ggsave("TSFP_HISTOGRAM_HULLAHALLI_facet.png", width = 6, height = 10)
ggsave("TSFP_HISTOGRAM_HULLAHALLI_Facet.pdf", width = 6, height = 10)
