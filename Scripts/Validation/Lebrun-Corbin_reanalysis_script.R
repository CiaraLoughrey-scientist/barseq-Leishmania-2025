...
#lebrun corbin data used for this re-analysis#
...
barcode_counts5 <- read.csv("Frequencies_M1toM13.csv")
...
INPUTS_VEC <- colnames(barcode_counts6[1:26])
...
ggplot(subset(df_long3, direction != "NA"), aes(x = log(new_FFP))) + 
  ...
  geom_vline(xintercept = 3, size = 1, linetype = "dashed", colour = "red") +
...
ggsave("FFP_HISTOGRAM_lebrun_facet.png", width = 6, height = 10)
ggsave("FFP_HISTOGRAM_lebrun_facet.pdf", width = 6, height = 10)
...
ggplot(subset(df_long2, direction != "NA"), aes(x = log(new_FP))) + 
  ...
  theme_bw() + theme(legend.title = element_text(size = 20),
                     ...
                     strip.text = element_text(size = 20)) +
  facet_wrap(~direction, nrow = 2, , labeller = as_labeller(c("1TRUE" = "True: A to B", "2BACK" = "Back: B to A")))
ggsave("TSFP_HISTOGRAM_lebrun_facet.png", width = 6, height = 10)
ggsave("TSFP_HISTOGRAM_lebrun_facet.pdf", width = 6, height = 10)
